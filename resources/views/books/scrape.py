...
async def fetch_books_for_keywords(keywords):
    combined_results = []
    ...
    async with aiohttp.ClientSession() as session:
        ...
        tasks = [fetch_and_parse_books(keyword, session) for keyword in keywords]
        results = await asyncio.gather(*tasks)
        combined_results = [book for sublist in results for book in sublist]
...
async def main():
    keywords = ['Dag Heward Mills', 'C.S Lewis', 'Derek Prince', 'Christian']  # List of keywords
    results = await fetch_books_for_keywords(keywords)
